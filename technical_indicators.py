...
def fetch_technical_indicators(candles, ema_windows=[5, 9, 12, 15, 21, 50, 100, 200], sma_windows=[10, 20, 50, 100, 200], 
    ...
    df["STOCH_k"] = pta.stoch(df["high"], df["low"], df["close"], k=14, d=3).iloc[:, 0].fillna(50)
    df["STOCH_d"] = pta.stoch(df["high"], df["low"], df["close"], k=14, d=3).iloc[:, 1].fillna(50)
    df["Fast_STOCH_k"] = pta.stoch(df["high"], df["low"], df["close"], k=5, d=3).iloc[:, 0].fillna(50)
    df["Fast_STOCH_d"] = pta.stoch(df["high"], df["low"], df["close"], k=5, d=3).iloc[:, 1].fillna(50)
    ...
    df["MACD_Signal"] = macd.iloc[:, 1].fillna(0)
    df["MACD_Hist"] = macd.iloc[:, 2].fillna(0)
    vwap = ((df["high"] + df["low"] + df["close"]) / 3 * df["volume"]).cumsum() / df["volume"].cumsum()
    vwap_macd = pta.macd(vwap)
    df["VWMACD"] = vwap_macd.iloc[:, 0].fillna(0)
    df["VWMACD_Signal"] = vwap_macd.iloc[:, 1].fillna(0)
    df["VWMACD_Hist"] = vwap_macd.iloc[:, 2].fillna(0)
    ...
    df["Stoch_RSI_14"] = pta.stochrsi(df["close"], length=14).iloc[:, 0].fillna(0.5)
    df["Stoch_RSI_9"] = pta.stochrsi(df["close"], length=9).iloc[:, 0].fillna(0.5)
    ...
    df["Klinger"] = pta.kvo(df["high"], df["low"], df["close"], df["volume"], short=34, long=55).iloc[:, 0].fillna(0)
    df["TRIX_15"] = pta.trix(df["close"], length=15).iloc[:, 0].fillna(0) * 100
    
    # --- Volatility Indicators ---
    df["ATR_14"] = pta.atr(df["high"], df["low"], df["close"], length=14).fillna(0)
    bb = pta.bbands(df["close"], length=20, std=2)
    df["BB_upper"] = bb.iloc[:, 2].fillna(df["close"])
    df["BB_lower"] = bb.iloc[:, 0].fillna(df["close"])
    kc = pta.kc(df["high"], df["low"], df["close"], length=20)
    df["KC_upper"] = kc.iloc[:, 2].fillna(df["close"])
    df["KC_lower"] = kc.iloc[:, 0].fillna(df["close"])
    dc = pta.donchian(df["high"], df["low"], length=20)
    df["DC_upper"] = dc.iloc[:, 2].fillna(df["close"])
    df["DC_lower"] = dc.iloc[:, 0].fillna(df["close"])
    ...
    df["Vortex_Pos"] = pta.vortex(df["high"], df["low"], df["close"], length=14).iloc[:, 0].fillna(0.5)
    df["Vortex_Neg"] = pta.vortex(df["high"], df["low"], df["close"], length=14).iloc[:, 1].fillna(0.5)
    df["Vortex_Pos_20"] = pta.vortex(df["high"], df["low"], df["close"], length=20).iloc[:, 0].fillna(0.5)
    df["Vortex_Neg_20"] = pta.vortex(df["high"], df["low"], df["close"], length=20).iloc[:, 1].fillna(0.5)
    df["Aroon_Up"] = pta.aroon(df["high"], df["low"], length=14).iloc[:, 1].fillna(50)
    df["Aroon_Down"] = pta.aroon(df["high"], df["low"], length=14).iloc[:, 0].fillna(50)
