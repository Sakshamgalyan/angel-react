...
try:
    ...
    logger.warning(f"Google API imports failed: {e} - Google Sheets integration will be disabled")
...
# Authenticate with Angel (wrap to log failures during import-time auth)
try:
    smartApi, jwt_token, feed_token = authenticate()
except Exception as e:
    # During deployment envs we may not have credentials or network access; log and re-raise so the
    # server startup fails with a clear message rather than causing a generic 500 later.
    logger.exception("Angel authentication failed at startup")
    raise
...
# Google Sheets setup (guard against module or creds errors on startup)
...
try:
    ...
    logger.exception("Google Sheets initialization failed. App will continue but append_to_sheet will error if used.")
    sheets_service = None
    sheet = None
...
def append_to_sheet(range_name: str, values: List[List[Any]]):
    # In TEST_MODE we skip Google Sheets writes
    if os.environ.get("TEST_MODE", "false").lower() == "true":
        logger.info("TEST_MODE enabled - skipping append_to_sheet")
        return
    if sheet is None:
        logger.error("append_to_sheet called but Google Sheets is not initialized - check credentials and dependencies")
